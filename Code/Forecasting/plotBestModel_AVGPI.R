...
plotModel <- function(model_damning, model_damhusaen, from, to){
  ...
  plotMulti <- function(data_plot, ymin, ymax, title){
    ...
    ggplot(data_plot, aes(x = as.POSIXct(index(data_plot))))+
      geom_rect(data=in_R, inherit.aes = F,aes(xmin=as.POSIXct(x1), xmax=as.POSIXct(x2), ymin=y1, ymax=y2), 
                fill='#6bdcff', colour = "#00c3ff", alpha=0.3,color=NA) +
      geom_point(aes(y = forecast90, color = "90 minutes forecast"), size = 0.7) + 
      geom_line(aes(y = forecast90, color = "90 minutes forecast", group = lastvalue90), size = 0.7, linetype="solid", alpha = 0.7) +
      geom_point(aes(y = forecast60, color = "60 minutes forecast"), size = 0.7) + 
      geom_line(aes(y = forecast60, color = "60 minutes forecast", group = lastvalue60), size = 0.7, linetype="solid", alpha = 0.7) +
      geom_point(aes(y = forecast30, color = "30 minutes forecast"), size = 0.7) + 
      geom_line(aes(y = forecast30, color = "30 minutes forecast", group = lastvalue30), size = 0.7, linetype="solid", alpha = 0.7) +
      geom_point(aes(y = station, color = "Measured"), size = 0.5) + 
      geom_line(aes(y = station, color = "Measured"), size = 0.5, linetype="dotted") +
      scale_color_manual(values=c("#e33307", "#ffaa00", "#7ce307", "black"))+
  ...
  #damhusaenList <- lapply(X=(1:5), function(i){ damhusaenList[[i]]})
  
  data_dammning <- dammningList[[model_damning]]
  data_damhusaen <- damhusaenList[[model_damhusaen]]
  ...
  if (External_Regressors_dammning){
    title_dammning <- paste("Dæmningen, ", type_damningen, " (", order_dammning[1], ", ", order_dammning[2], ", ", order_dammning[3], ", ", reg.nr_dammning, ", ", reg.lag_dammning, ")", sep = "")
  }else{title_dammning <- paste("Dæmningen, ", type_damningen, " (", order_dammning[1], ", ", order_dammning[2], ", ", order_dammning[3], ")", sep = "")}
  
  if (External_Regressors_damhusaen){
    title_damhusaen <- paste("Damhusåen, ", type_damhusaen, " (", order_damhusaen[1], ", ", order_damhusaen[2], ", ", order_damhusaen[3], ", ", reg.nr_damhusaen, ", ", reg.lag_damhusaen, ")", sep = "")
  }else{title_damhusaen <- paste("Damhusåen, ", type_damhusaen, " (", order_damhusaen[1], ", ", order_damhusaen[2], ", ", order_damhusaen[3], ")", sep = "")}
...
model_damning <- 2
model_damhusaen <- 3
plot <- plotModel(model_damning = model_damning, model_damhusaen = model_damhusaen, from = from, to = to)
plot

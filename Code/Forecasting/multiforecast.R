...
order_S1 <- c(7,1,5)
order_S2 <- c(7,1,1)
coeff_S1 <- c(0.0363205088492721, 0.186638636147475, -0.285262513704381, 
                  0.0711980655059393, 0.0100222529278041, 0.141209763277267, -0.123421767191628, 
                  0.395103668865874, -0.0534508046264153, 0.104602531827269, 0.677085031046783, 
                  0.50485755532366, 0.219026851915658, 0.297590565577813, -0.261745709813947, 
                  -0.130557949614868)
coeff_S2 <- c(0.172302584547302, -0.106561444152176, 0.0694964695275962, 
  -0.219596357294935, 0.25568875249389, -0.0553348874111356, -0.0612899178992514, 
  0.283414810225867, 0.151953058657329, 0.0749672957883627, 0.135265008107362, 
  -0.080437109738914)
...
from <- which(index(data) == as.character("2018-03-09 00:10:00"))
to <- which(index(data) == as.character("2018-03-09 12:00"))
...
plotMulti <- function(data_plot, ymin, ymax, title){
  ...
  ggplot(data_plot, aes(x = as.POSIXct(index(data_plot))))+
    ...
    geom_point(aes(y = forecast90, color = "90 minutes forecast"), size = 0.4) + 
    geom_line(aes(y = forecast90, color = "90 minutes forecast", group = lastvalue90), linetype="solid") +
    geom_point(aes(y = forecast60, color = "60 minutes forecast"), size = 0.4) + 
    geom_line(aes(y = forecast60, color = "60 minutes forecast", group = lastvalue60), linetype="solid") +
    geom_point(aes(y = forecast30, color = "30 minutes forecast"), size = 0.4) + 
    geom_line(aes(y = forecast30, color = "30 minutes forecast", group = lastvalue30), linetype="solid") +
    geom_point(aes(y = station, color = "Measured"), size = 0.4, alpha = 0.4) + 
    geom_line(aes(y = station, color = "Measured"), alpha = 0.4) +
    scale_color_manual(values=c("#C70039", "#FF5733", "#FFC300", "black"))+
    ...
    theme(panel.grid.major = element_line(size=.20,colour = "grey50"),
          ...
          plot.title = element_text(size = 26), text = element_text(size=22),
...
p1 <- plotMulti(data_plot_S1, ymin, ymax, title = "Damning")
p2 <- plotMulti(data_plot_S2, ymin, ymax, title = "Damhusaen")

plot <- ggarrange(p1, p2, ncol = 1, nrow = 2, common.legend = TRUE, legend = "bottom")
plot <- annotate_figure(plot, top = text_grob(paste("Forecasts on ", as.Date(index(data_plot_S1)[1]), sep = ""), size = 30, vjust = 1))





svg(filename = "../Figures/Results/real_life_forecast.svg", height= 10, width = 11)
plot
dev.off()


 ## save as separate files and then join in 
# svg(filename = "../Figures/Results/real_life_forecast_dammning.svg")
# p1
# dev.off()
# 
# svg(filename = "../Figures/Results/real_life_forecast_damhusaen.svg")
# p2
# dev.off()

...
meta_optim <- function(order, External_Regressor, reg.nr, reg.lag){
  ...
  optimize_arima=function(pars,order, Xreg, reg.nr){
    ...
    residuals <- unlist(output$residuals[1])

    ss <- sum((residuals[wwIndex == 1])^2, na.rm = T) 
    return(ss) 
  }
  templist <- list()
  n <- order[1]+order[3]
  ...
  if ((order[1] > 0) || (order[3] > 0)){
    ...
    while (counter <= max_counter){
      ...
      results = try(optim(par = startpars, fn = optimize_arima, order=order, reg.nr = reg.nr, Xreg = reg, method = "Nelder-Mead", control = list(maxit = 1000)), silent = TRUE) #

...
meta_optim <- function(order, External_Regressor, reg.nr, reg.lag){
  ...
  evaluate <- function(ts, order, coefficients, Xreg){
    output <- multistepARIMA(ts = ts_validation, order = order, coefficients = results$par, Xreg = reg)
    pred <- unlist(output$predictions[3])
    pred <- norm_const_station * pred
    p1 = pred
    runoff_validation <- norm_const_station * ts_validation
    data <- as.data.frame(cbind(as.character(timestamp_validation), runoff_validation, pred), stringsAsFactors=FALSE)
    ...
    return(list(evaluation30, evaluation60, evaluation90))
  }
  ...
  F1_score <- function(evaluation){
    ### F1 score does not take early into account.
    
    TP <- evaluation$correct
    FN <- evaluation$missed
    FP <- evaluation$false_alarm
    #TN <- 
    #precision = TP / (TP + FN)
    sen <- TP/(TP + FN)
  ...
  if ((order[1] > 0) || (order[3] > 0)){
    ...
    while (counter <= max_counter){
      ...
      results = try(optim(par = startpars, fn = optimize_arima, order=order, reg.nr = reg.nr, Xreg = reg, method = "Nelder-Mead"), silent = TRUE) #
    ...
    if (counter >= max_counter){
      ...
      L <- evaluate(ts = ts_validation, order = order, coefficients = results_list[[where_min_value]]$par, Xreg = reg)
      evaluation30 <-  L[[1]] 
      evaluation60 <-  L[[2]] 
      evaluation90 <-  L[[3]] 
      ...
      list_return <- c(list(order = order), 
                       ...
                       evaluation30 = list(evaluation30), 
                       evaluation60 = list(evaluation60),
                       evaluation90 = list(evaluation90),
                       F1_30 = F1_score(evaluation30),
                       F1_60 = F1_score(evaluation60),
                       F1_90 = F1_score(evaluation90))
      list_return$message <- paste((max_counter)," tries: failed to converge")
      
      templist <- list_return
    } else{
      ## Instead of finding LL, we produce 60 min forecast for 4 monts after and find the validation error
      ## This validation error will be used for evaluating model perfomance.
      
      L <- evaluate(ts = ts_validation, order = order, coefficients = results$par, Xreg = reg)
      evaluation30 <-  L[[1]] 
      evaluation60 <-  L[[2]] 
      evaluation90 <-  L[[3]] 

...
meta_optim_ml=function(orders, External_Regressor, reg.nr, reg.lag){
  exit <- function() {
  ...
  uRoot_test <- function(order, pars){
    ...
    if (p > 0){
      AR <- pars[1:p]
      AR_flag <- !all(abs(polyroot(c(1,-AR))) > 1) ## If AR_flag is 1, the AR polynominal roots are not stationary
    }else{AR_flag = FALSE}
    if (q > 0){
      MA <- pars[((p+1):(p+q))]
    ...
    if (!is.null(reg.nr)){
      ...
      reg_flag <-  !all(abs(polyroot(c(1, -reg))) > 1)
    }else{reg_flag = FALSE}
  ...
  multistepARIMA <- function(order, coefficients, Xreg){
  ...
  optimize_arima <- function(pars,order, Xreg, reg.nr){
    ...
    output <- multistepARIMA(order = order, coefficients = pars, Xreg = Xreg)
    ...
    ## maximum likelihood
    mu <- mean(SC_, na.rm = T)
    sigma <- sd(SC_, na.rm = T)
    n <- length(SC_)
    LL <-  (- 0.5*n*log(2*pi) - 0.5*n*log(sigma^2) - (0.5/(sigma^2)) * sum((SC_[wwIndex == 1])^2, na.rm = T))
    #we want to maximize LL so we return the negative LL (because optim minimizes)
    return(-LL) 
  }
  ...
  if ((orders[1] > 0) || (orders[3] > 0)){
    startpars <- vector(length = n)
    # sd <- sample(0:4, 1)
    # startpars <- rnorm(n = n, mean = 0, sd = sd)
    # startpars
    # results = optim(par = startpars, fn = optimize_arima, order=orders, reg.nr = reg.nr, Xreg = reg, method = "L-BFGS-B", lower = rep(-2,n), upper = rep(2,n), control = list(maxit = 1000))
    counter <- 1
    max_counter <- 50
      while (counter <= max_counter){
        uRoot <- 1
        while (uRoot == 1){
          startpars <- rnorm(n = n, mean = 0, sd = sample(0:4, 1))
          uRoot <- uRoot_test(order, startpars)
        }
        #startpars <- rnorm(n = n, mean = 0, sd = sample(0:4, 1))
        results = try(optim(par = startpars, fn = optimize_arima, order=orders, reg.nr = reg.nr, Xreg = reg, method = "Nelder-Mead", control = list(maxit = 500)), silent = TRUE) #
        uRoot <- uRoot_test(order, results$par)
        if (is(results, 'try-error') || (results$convergence != 0) || (uRoot == 1)) ## Or charecteristic roots are inside the unit circle
        {
          #print(paste("Try nr:", counter))
          counter = counter + 1
        } else{
          break;
        }
    ...
    if (counter >= max_counter){
      templist <- list(order = orders, message = paste(counter," tries: failed to converge"))
    } else{
        LL <- -results$value
        AIC <-  2 * n - 2 * log(LL)
        templist <- list(order = orders, 
                       parameters = results$par, 
                       value = results$value, 
                       tries = counter,
                       iterations = results$counts, 
                       convergence = results$convergence,
                       message = results$message, 
                       loglikelihood = LL, 
                       AIC = AIC)
      }
    } else{ templist <- list(order = orders, message = "No parameters to optimize for")}
  return(templist)
}

...
meta_optim_mcss <- function(order, External_Regressor, reg.nr, reg.lag){
  exit <- function() {
  ...
  loglik_arima <- function(pars, order, Xreg, reg.nr){
  ...
  if ((order[1] > 0) || (order[3] > 0)){
    ...
    # startpars
    # results = optim(par = startpars, fn = optimize_arima, order=order, reg.nr = reg.nr, Xreg = reg, method = "L-BFGS-B", lower = rep(-2,n), upper = rep(2,n), control = list(maxit = 1000))
    counter <- 1
    max_counter <- 50
    results_list <- list()
      while (counter <= max_counter){
        uRoot <- 1
        while (uRoot == 1){
          startpars <- rnorm(n = n, mean = 0, sd = sample(1:4, 1))
          uRoot <- uRoot_test(order, startpars)
        }
        #startpars <- rnorm(n = n, mean = 0, sd = sample(0:4, 1))
        results = try(optim(par = startpars, fn = optimize_arima, order=order, reg.nr = reg.nr, Xreg = reg, method = "Nelder-Mead", control = list(maxit = 500)), silent = TRUE) #
    ...
    if (counter >= max_counter){
      ...
      list_return <- list(order = order, results_list[[where_min_value]])
      list_return$message <- paste((max_counter)," tries: failed to converge")
      templist <- list_return
      
    } else{
      ## Evaluate differently
      LL <- loglik_arima(pars = results$par, order = order, Xreg = reg, reg.nr = reg.nr)
      AIC <-  2 * n - 2 * (LL)

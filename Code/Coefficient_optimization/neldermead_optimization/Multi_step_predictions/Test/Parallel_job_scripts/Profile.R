...
meta_optim=function(orders, external_regressor){
  ...
  compute_arima_mse=function(pars,order){
  ...
  if (n>0){
    ...
    while (counter <= max_counter){
      ...
      results = try(optim(par = startpars, fn = compute_arima_mse, order=orders), silent = TRUE) #
...
system.time(
  compute_arima_mse(order = orders, pars = startpars)
)
...
startpars=  vector(length = n)
sd <- sample(2:4, 1)
startpars <- rep(0, n) #rnorm(n = n, mean = 0, sd = sd)
system.time(
  nlm(f = compute_arima_mse, p = startpars, order = orders)
)



system.time(
  res <- optim(par = startpars, fn = compute_arima_mse, order=orders, control = list(maxit = 1000))
)
res

system.time(
  out <- meta_optim(orders = orders, external_regressor = FALSE)
)

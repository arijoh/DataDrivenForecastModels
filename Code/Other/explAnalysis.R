...
start = which(as.POSIXct(wwS1$Timestamp, format="%Y/%m/%d %H:%M", tz = "GMT") == from)
end = which(as.POSIXct(wwS1$Timestamp, format="%Y/%m/%d %H:%M", tz = "GMT") == to)
...
plotTS_zoom_wwindex <- function(data, from, wwindex, to, ymin, ymax, title){
  ...
  plot <- ggplot(data, aes(x = as.POSIXct(Timestamp, format="%Y-%m-%d %H:%M")))+
    ...
    geom_line(aes(y = Runoff), color = "#10C312", size = 0.25)+
    scale_x_datetime(breaks = date_breaks("24 hours"), date_labels = "%Y-%m-%d")+
    scale_y_continuous(limits = yl, expand = c(0, 0), breaks=seq(yl[1],yl[2],10000),
                       sec.axis = sec_axis(~./scalingFactor, name = "Rain [mm/hr]          ", breaks=yTick,labels =ylab))+
    facet_grid(rows = vars(Station))+
    ylab("Runoff [m3/hr]")+xlab("Date")+
    ggtitle(title)+
    theme_pubclean()+
    theme(panel.grid.major = element_line(size=.20,colour = "grey50"),
          panel.grid.minor = element_blank(),
...
title <- "Flow measurements and precipitation"
plot_wwindex <- plotTS_zoom_wwindex(data = data, wwindex = ww,from = from, to = to, ymin = ymin, ymax = ymax, title = title)
plot_wwindex




ggsave(filename="../Figures/zoom_plotwwindex.pdf", units = "mm", width = 90, height = 90, limitsize = FALSE)
full_plot_wwindex
dev.off()

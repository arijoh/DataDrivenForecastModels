...
### We have 4 lists with all the info for
###   Dammningen - Nelder-Mead models
###  Dammningen - DDS models
###   Damhusaen - Nelder-Mead models
###  Damhusaen - DDS models
...
Damningen_NM_PI <- fillData_PI(DamningenList_NM)
Damningen_NM_Accuracy <- fillData_Accuracy(DamningenList_NM)

Damhusaen_NM_PI <- fillData_PI(DamhusaenList_NM)
Damhusaen_NM_Accuracy <- fillData_Accuracy(DamhusaenList_NM)

Damningen_DDS_PI <- fillData_PI(DamningenList_DDS)
Damningen_DDS_Accuracy <- fillData_Accuracy(DamningenList_DDS)

Damhusaen_DDS_PI <- fillData_PI(DamhusaenList_DDS)
Damhusaen_DDS_Accuracy <- fillData_Accuracy(DamhusaenList_DDS)
...
findBestScore <- function(df){
  ...
  for (i in (1:3)){
    data_return[i,] <- findBestScore_(df, i)[1,]
...
#### Get best models
Damningen_NM_PI <- findBestScore(Damningen_NM_PI)
Damningen_DDS_PI <- findBestScore(Damningen_DDS_PI)

Damhusaen_NM_PI <- findBestScore(Damhusaen_NM_PI)
Damhusaen_DDS_PI <- findBestScore(Damhusaen_DDS_PI)

Damningen_NM_Accuracy <- findBestScore(Damningen_NM_Accuracy)
Damningen_DDS_Accuracy <- findBestScore(Damningen_DDS_Accuracy)

Damhusaen_NM_Accuracy <- findBestScore(Damhusaen_NM_Accuracy)
Damhusaen_DDS_Accuracy <- findBestScore(Damhusaen_DDS_Accuracy)


makedf <- function(S1_NM, S1_DDS, S2_NM, S2_DDS){
  S1_NM$Optimization <- rep("Nelder-Mead")
  S1_DDS$Optimization <- rep("DDS")
  S1 <- rbind(S1_NM, S1_DDS)
  
  S2_NM$Optimization <- rep("Nelder-Mead")
  S2_DDS$Optimization <- rep("DDS")
  S2 <- rbind(S2_NM, S2_DDS)
...
dataPI <- makedf(S1_NM = Damningen_NM_PI, 
               S1_DDS = Damningen_DDS_PI, 
               S2_NM = Damhusaen_NM_PI,
               S2_DDS = Damhusaen_DDS_PI)
dataAccuracy <- makedf(S1_NM = Damningen_NM_Accuracy, 
                S1_DDS = Damningen_DDS_Accuracy, 
                S2_NM = Damhusaen_NM_Accuracy,
                S2_DDS = Damhusaen_DDS_Accuracy)
...
plot <- ggplot(data, aes(x = sort, y = value, shape = Optimization, colour = variable, group = interaction(sort, variable)))+
  ...
  ggtitle("Best performing models based on Accuracy/PI\nfor 30 min, 60 min, and 90 min forecasting horizon")+
...
ggsave(filename = "../Figures/Results/Comparison/Evaluation/best_models_optim.pdf", width = 8, height = 5)
plot
dev.off()

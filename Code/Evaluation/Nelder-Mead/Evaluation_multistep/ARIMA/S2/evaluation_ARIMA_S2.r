...
files <- list.files(path = "Coefficient_optimization/Multi_step_predictions/Job_splitting/ARIMA/S2/Lists", pattern = ".rdata", recursive = TRUE)
ARIMA_S2 <- lapply(paste("Coefficient_optimization/Multi_step_predictions/Job_splitting/ARIMA/S2/Lists/",files, sep = ""), function(x) mget(load(x))$results)
...
assign(name, results)
filename <- paste("Evaluation/Evaluation_multistep/ARIMA/S2/Lists/", name, ".rdata",sep = "")

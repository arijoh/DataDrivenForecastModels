...
getReg <- function(External_Regressor, L){
...
evaluate <- function(data){
  ...
  data_test <- as.data.frame(cbind(as.character(index(data)), coredata(data$observation), coredata(data$forecast30)))#, stringsAsFactors=FALSE)
  colnames(data_test) <- c("Timestamp" , "Meas", "pred")
  evaluation30 <- evaluateModel(data_test)
  
  
  data_test <- as.data.frame(cbind(as.character(index(data)), coredata(data$observation), coredata(data$forecast60)))#, stringsAsFactors=FALSE)
  colnames(data_test) <- c("Timestamp" , "Meas", "pred")
  evaluation60 <- evaluateModel(data_test)
  
  
  data_test <- as.data.frame(cbind(as.character(index(data)), coredata(data$observation), coredata(data$forecast90)))#, stringsAsFactors=FALSE)
  colnames(data_test) <- c("Timestamp" , "Meas", "pred")
  evaluation90 <- evaluateModel(data_test)
...
PI <- function(data){
  ...
  over <- (data$observation-data$forecast30)^2
  under <- (data$observation-data$lastvalue30)^2
  PI30 <- 1 - sum(over[wwIndex == 1], na.rm = T)/sum(under[wwIndex == 1], na.rm = T)
  
  over <- (data$observation-data$forecast60)^2
  under <- (data$observation-data$lastvalue60)^2
  PI60 <- 1 - sum(over[wwIndex == 1], na.rm = T)/sum(under[wwIndex == 1], na.rm = T)
  
  over <- (data$observation-data$forecast90)^2
  under <- (data$observation-data$lastvalue90)^2
  PI90 <- 1 - sum(over[wwIndex == 1], na.rm = T)/sum(under[wwIndex == 1], na.rm = T)
  
  return(list(PI30 = PI30, 
              PI60 = PI60, 
              PI90 = PI90))
}
evaluateLists <- function(L){
  #for (nr in (1:length(L))){
    #print(nr)
    #order <- L[[nr]]$order
    #coefficients <- L[[nr]]$par
    order <- L$order
    coefficients <- L$par
    
    Xreg <- getReg(External_Regressor = External_Regressor, L)
    ...
    if (!is.null(coefficients)){
      ...
      L$evaluation <- list(evaluation30 = E$evaluation30,
                                evaluation60 = E$evaluation60,
                                evaluation90 = E$evaluation90)
      
      L$accuracy <- list(accuracy30 = model_accuracy(E$evaluation30),
                              accuracy60 = model_accuracy(E$evaluation60),
                              accuracy90 = model_accuracy(E$evaluation90))
              
      L$PI <-  PI(data)
      
      L$RMSE <- list(RMSE30 = sqrt(sum((data$observation - data$forecast30)^2, na.rm = T)/length(data$observation)),
                          RMSE60 = sqrt(sum((data$observation - data$forecast60)^2, na.rm = T)/length(data$observation)),
                          RMSE90 = sqrt(sum((data$observation - data$forecast90)^2, na.rm = T)/length(data$observation)))
      
      L$MAE <- list(MAE30 = sum(abs(data$observation - data$forecast30), na.rm = T)/length(data$observation),
                         MAE60 = sum(abs(data$observation - data$forecast60), na.rm = T)/length(data$observation),
                         MAE90 = sum(abs(data$observation - data$forecast90), na.rm = T)/length(data$observation))

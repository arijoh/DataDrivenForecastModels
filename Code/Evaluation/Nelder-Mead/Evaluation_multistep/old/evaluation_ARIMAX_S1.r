...
wd <- getwd()
source(paste(wd,"/Evaluation/Evaluation_multistep/functions.r", sep = ""))
files <- list.files(path = "Coefficient_optimization/Multi_step_predictions/Job_splitting/ARIMAX/S1/Lists", pattern = ".rdata", recursive = TRUE)
ARIMAX_S1 <- lapply(paste("Coefficient_optimization/Multi_step_predictions/Job_splitting/ARIMAX/S1/Lists/",files, sep = ""), function(x) mget(load(x))$results)
...
results <- evaluateLists(ARIMAX_S1[[myargument]])


save(ARIMAX_S1_ordered, file = "Evaluation/Evaluation_multistep/Lists/ARIMAX_S1.rdata")
name <- paste("eval_ARIMAX_s1_c(", order[1], ", ", order[2], ", ", order[3], ", ", "reg.nr=", reg.nr, "reg.lag=", reg.lag,  ")", sep = "")
assign(name, results)

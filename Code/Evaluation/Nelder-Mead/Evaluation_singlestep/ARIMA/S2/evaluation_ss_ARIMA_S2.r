...
files <- list.files(path = "Coefficient_optimization/Single_step_predictions/HPC/ARIMA/S2/Lists", pattern = ".rdata", recursive = TRUE)
ARIMA_S2 <- lapply(paste("Coefficient_optimization/Single_step_predictions/HPC/ARIMA/S2/Lists/",files, sep = ""), function(x) mget(load(x))$results)
...
name <- paste("eval_ARIMA_ss_S2_c(", X$order[1], ", ", X$order[2], ", ", X$order[3], ", ", "reg.nr=", X$reg.nr, "reg.lag=", X$reg.lag,  ")", sep = "")
assign(name, results)
filename <- paste("Evaluation/Evaluation_singlestep/ARIMA/S2/Lists/", name, ".rdata",sep = "")

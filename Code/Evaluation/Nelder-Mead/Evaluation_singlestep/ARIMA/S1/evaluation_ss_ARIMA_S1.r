...
files <- list.files(path = "Coefficient_optimization/Single_step_predictions/HPC/ARIMA/S1/Lists", pattern = ".rdata", recursive = TRUE)
ARIMA_S1 <- lapply(paste("Coefficient_optimization/Single_step_predictions/HPC/ARIMA/S1/Lists/",files, sep = ""), function(x) mget(load(x))$results)
...
assign(name, results)
filename <- paste("Evaluation/Evaluation_singlestep/ARIMA/S1/Lists/", name, ".rdata",sep = "")

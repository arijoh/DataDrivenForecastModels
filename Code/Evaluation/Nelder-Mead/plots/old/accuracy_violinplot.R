...
plotViolin <- function(data, title){
  plot <- ggplot(df, aes(x = variable, y = value, fill = Type, group = interaction(Type, variable))) +
    ...
    xlab("Accuracy")+ylab("Accuracy of correct ATS events")+
    theme_minimal()+
...
svg(filename = "../Figures/Results/Nelder-Mead/better_plots?/Accuracy_violinplot_2.svg", width = 8, height = 5)
plot
dev.off()

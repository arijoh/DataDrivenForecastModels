...
ARIMAX_S2 <- lapply(paste("Coefficient_optimization/dds_optimization/Multi_step_predictions/ARIMAX/S2/Lists/",files, sep = ""), function(x) mget(load(x))$results)
...
name <- paste("eval_ARIMAX_S2_c(", X$order[1], ", ", X$order[2], ", ", X$order[3], ", ", "reg.nr=", X$reg.nr, "reg.lag=", X$reg.lag,  ")", sep = "")
assign(name, results)
filename <- paste("Evaluation/DDS/Evaluation_multistep/ARIMAX/S2/Lists/", name, ".rdata",sep = "")

...
X <- ARIMAX_S2[[myargument]]
results <- evaluateLists(X)


name <- paste("eval_ARIMAX_ss_S2_c(", X$order[1], ", ", X$order[2], ", ", X$order[3], ", ", "reg.nr=", X$reg.nr, "reg.lag=", X$reg.lag,  ")", sep = "")
assign(name, results)
filename <- paste("Evaluation/DDS/Evaluation_singlestep/ARIMAX/S2/Lists/", name, ".rdata",sep = "")

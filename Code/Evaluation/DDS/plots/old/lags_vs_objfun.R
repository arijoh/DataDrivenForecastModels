...
plot_dds <- ggplot(data, aes(x = reg.lag, y = objval, fill = station, group = interaction(station, reg.lag)))+
  geom_violin()+
...
plot <- annotate_figure(ggarrange(plot_nm, plot_dds, nrow = 2), 
                top = text_grob("Lag of regressor and obj.fun. value", size = 16))
plot


svg(filename = "../Figures/Results/lag_vs_objfun.svg")
plot
dev.off()

# svg(filename="../Figures/Results/DDS/reglag_vs_objfun.svg", width = 8, heigh = 4)
# plot
# dev.off()
